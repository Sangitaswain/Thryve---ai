...
const App: React.FC = () => {
  ...
  const renderScreen = () => {
    switch (activeTab) {
      case 'home': return <div key="home" className="animate-in fade-in slide-in-from-bottom-2 duration-500"><HomeScreen onNavigateToChat={() => setActiveTab('chat')} /></div>;
      case 'track': return <div key="track" className="animate-in fade-in slide-in-from-bottom-2 duration-500"><TrackScreen /></div>;
      case 'insights': return <div key="insights" className="animate-in fade-in slide-in-from-bottom-2 duration-500"><InsightsScreen /></div>;
      case 'profile': return <div key="profile" className="animate-in fade-in slide-in-from-bottom-2 duration-500"><ProfileScreen /></div>;
      case 'chat': return <div key="chat" className="animate-in fade-in duration-300"><ChatScreen onBack={() => setActiveTab('home')} /></div>;
      default: return <HomeScreen onNavigateToChat={() => setActiveTab('chat')} />;
    }
  };

  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#F7F9FB]">
      <main className="flex-1 overflow-y-auto pb-24 scroll-smooth">
        {renderScreen()}
      </main>

      <nav className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto bg-white/90 backdrop-blur-xl border-t border-slate-100 flex justify-around items-center py-4 px-2 z-50 rounded-t-[2.5rem] shadow-[0_-8px_30px_rgb(0,0,0,0.03)]">
        <TabButton 
        ...
        <div className="relative -mt-10">
          <button 
            ...
            className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-all active:scale-90 ${activeTab === 'chat' ? 'bg-[#4CB8A4] text-white shadow-[#4CB8A4]/20' : 'bg-[#1F2933] text-white shadow-slate-200'}`}
          >
...
const TabButton: React.FC<TabButtonProps> = ({ isActive, onClick, label, icon }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center gap-0.5 transition-all active:scale-95 ${isActive ? 'text-[#4CB8A4]' : 'text-[#6B7280] hover:text-[#1F2933]'}`}
  >
